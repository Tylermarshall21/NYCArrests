...
```{r}
nyc_arrests_type <- nyc_arrests2 %>%
  group_by(LAW_CA) %>%
  summarize(Number_Arrests = n())

ggplot(data = nyc_arrests_type, aes(x = fct_reorder(LAW_CA, Number_Arrests, .desc = TRUE), y = Number_Arrests)) + geom_col()


nyc_arrests_year <- nyc_arrests2 %>%
  mutate(ARREST_D = as_date(ARREST_D, format = "%m/%d/%Y"),
         ARREST_YEAR = format(ARREST_D, format = "%Y")) %>%
