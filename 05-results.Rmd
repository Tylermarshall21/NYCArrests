...
nyc_coords <- nyc_arrests_categories_simple %>%
  ...
  summarize(Murder = sum(Murder),
            ...
            Motor.Vehicle.Theft = sum(Motor.Vehicle.Theft),
            Robbery = sum(Robbery))
ggparcoord(data = nyc_coords, columns = 2:6, groupColumn = "County", scale = "globalminmax", splineFactor = 10) +
...
nyc_arrest_election <- nyc_arrests_combined %>%
  ...
  group_by(ARREST_YEAR) %>%
  summarize(Average_Arrests = n()/366)
...
ggmap(nyc, extent='device', legend='bottom') +
  ...
  geom_density_2d_filled(data = nyc_thanksgiving_arrests, aes(x = Longitude, y=Latitude), alpha = 0.4) +
  facet_wrap(~ THANKSGIVING)
```

The results are somewhat unexpected. It may seem intuitive to suspect elevated levels of crime around the parade route, but this does not seem to be the case. Many of the areas on the parade route, in fact, see little to no crime the day of the parade! We do note, however, a shift in the patterns of thefts on Thanksgiving day, with more thefts occurring on the Western side of the city than ordinary, with the area near Lincoln Center seeing a slightly higher increase. By contrast, many areas to the East of the parade route see a decline in thefts on Thanksgiving day. 
...
ggplot(data = drunk_driving, aes(x = ARREST_YEAR, y = Number_Arrests)) + geom_col() +
  theme_bw() +
