...
nyc_mj_arrests_by_date <- (
  nyc_arrests_combined %>%
    ...
    summarize(COUNT_OF_ARRESTS = n()) %>%
    mutate(month = make_date(year=ARREST_YEAR,month=ARREST_MONTH, day=15), ARREST_BORO = 'Total')
)
...
nyc_mj_arrests_by_date_boro <- (
  nyc_arrests_combined %>%
    filter(grepl("MARIJUANA", PD_DESC)) %>%
    mutate(ARREST_YEAR = year(ARREST_DATE), ARREST_MONTH = month(ARREST_DATE)) %>%
    group_by(ARREST_YEAR, ARREST_MONTH, ARREST_BORO) %>%
    summarize(COUNT_OF_ARRESTS = n()) %>%
...
ggplot(nyc_mj_arrests_by_date,aes(x=month,y=COUNT_OF_ARRESTS, color = ARREST_BORO)) +
  geom_line() +
  ...
  labs(title="Marijuana Arrests Over Time Aggregated by Month",
       x="Month",
       y="Count of Arrests",
...
nyc_thanksgiving_arrests$THANKSGIVING <- factor(nyc_thanksgiving_arrests$THANKSGIVING,
                                               levels= c(TRUE, FALSE),
                                               labels=c("Thanksgiving", "Thursday Before Thanksgiving")
                                               )
...
ggmap(nyc, extent='device', legend='bottom') +
  ...
  labs(title="Arrest Heatmap - Thanksgiving vs Week Before")
```
